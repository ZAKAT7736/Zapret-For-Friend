...
--filter-tcp=443 --ipset="ipset-telegram.txt" --dpi-desync=split2 --dpi-desync-any-protocol --dpi-desync-split-pos=2 --new ^
--filter-tcp=443 --hostlist="list-telegram.txt" --dpi-desync=fake,split --dpi-desync-autottl=2 --dpi-desync-repeats=6 --dpi-desync-fooling=badseq --dpi-desync-fake-tls="%BIN%tls_clienthello_www_google_com.bin" --new ^
